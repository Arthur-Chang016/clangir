...
void test_int_and_float(int *a, float *b) {
  ...
  // CIR: cir.scope
  // CIR: %[[TMP1:.*]] = cir.load {{.*}} %{{.*}} : !cir.ptr<!cir.ptr<!s32i>>, !cir.ptr<!s32i>
  // CIR: %[[TMP2:.*]] = cir.load deref {{.*}} %[[TMP1]] : !cir.ptr<!s32i>, !s32i tbaa(#tbaa[[INT]])
  // CIR: cir.if
  // CIR: %[[C2:.*]] = cir.const #cir.fp<2
  // CIR: %[[TMP3:.*]] = cir.load {{.*}} %[[ARG_b:.*]] : !cir.ptr<!cir.ptr<!cir.float>>, !cir.ptr<!cir.float>
  // CIR: cir.store{{.*}} %[[C2]], %[[TMP3]] : !cir.float, !cir.ptr<!cir.float> tbaa(#tbaa[[FLOAT]])
  // CIR: else
  // CIR: %[[C3:.*]] = cir.const #cir.fp<3
  // CIR: %[[TMP4:.*]] = cir.load {{.*}} %[[ARG_b]] : !cir.ptr<!cir.ptr<!cir.float>>, !cir.ptr<!cir.float>
...
void test_long_and_double(long *a, double *b) {
  ...
  // CIR: cir.scope
  // CIR: %[[TMP1:.*]] = cir.load {{.*}} %{{.*}} : !cir.ptr<!cir.ptr<!s64i>>, !cir.ptr<!s64i>
  // CIR: %[[TMP2:.*]] = cir.load deref {{.*}} %[[TMP1]] : !cir.ptr<!s64i>, !s64i tbaa(#tbaa[[LONG]])
  // CIR: cir.if
  // CIR: %[[C2:.*]] = cir.const #cir.fp<2
  // CIR: %[[TMP3:.*]] = cir.load {{.*}} %[[ARG_b:.*]] : !cir.ptr<!cir.ptr<!cir.double>>, !cir.ptr<!cir.double>
  // CIR: cir.store{{.*}} %[[C2]], %[[TMP3]] : !cir.double, !cir.ptr<!cir.double> tbaa(#tbaa[[DOUBLE]])
  // CIR: else
  // CIR: %[[C3:.*]] = cir.const #cir.fp<3
  // CIR: %[[TMP4:.*]] = cir.load {{.*}} %[[ARG_b]] : !cir.ptr<!cir.ptr<!cir.double>>, !cir.ptr<!cir.double>
...
void test_long_long_and_long_double(long long *a, long double *b) {
  ...
  // CIR: cir.scope
  // CIR: %[[TMP1:.*]] = cir.load {{.*}} %{{.*}} : !cir.ptr<!cir.ptr<!s64i>>, !cir.ptr<!s64i>
  // CIR: %[[TMP2:.*]] = cir.load deref {{.*}} %[[TMP1]] : !cir.ptr<!s64i>, !s64i tbaa(#tbaa[[LONG_LONG]])
  // CIR: cir.if
  // CIR: %[[C2:.*]] = cir.const #cir.fp<2
  // CIR: %[[TMP3:.*]] = cir.load {{.*}} %[[ARG_b:.*]] : !cir.ptr<!cir.ptr<!cir.long_double<!cir.f80>>>, !cir.ptr<!cir.long_double<!cir.f80>>
  // CIR: cir.store{{.*}} %[[C2]], %[[TMP3]] : !cir.long_double<!cir.f80>, !cir.ptr<!cir.long_double<!cir.f80>> tbaa(#tbaa[[LONG_DOUBLE]])
  // CIR: else
  // CIR: %[[C3:.*]] = cir.const #cir.fp<3
  // CIR: %[[TMP4:.*]] = cir.load {{.*}} %[[ARG_b]] : !cir.ptr<!cir.ptr<!cir.long_double<!cir.f80>>>, !cir.ptr<!cir.long_double<!cir.f80>>
...
void test_char(char *a, char* b) {
  ...
  // CIR: cir.scope
  // CIR: %[[TMP1:.*]] = cir.load {{.*}} %{{.*}} : !cir.ptr<!cir.ptr<!s8i>>, !cir.ptr<!s8i>
  // CIR: %[[TMP2:.*]] = cir.load deref {{.*}} %[[TMP1]] : !cir.ptr<!s8i>, !s8i tbaa(#tbaa[[CHAR]])
  // CIR: cir.if
  // CIR: %[[C2:.*]] = cir.const #cir.int<98> : !s32i
  // CIR: %[[C2_CHAR:.*]] = cir.cast(integral, %[[C2]] : !s32i), !s8i
  // CIR: %[[TMP3:.*]] = cir.load {{.*}} %[[ARG_b:.*]] : !cir.ptr<!cir.ptr<!s8i>>, !cir.ptr<!s8i>
  ...
  // CIR: %[[TMP4:.*]] = cir.load {{.*}} %[[ARG_b]] : !cir.ptr<!cir.ptr<!s8i>>, !cir.ptr<!s8i>
